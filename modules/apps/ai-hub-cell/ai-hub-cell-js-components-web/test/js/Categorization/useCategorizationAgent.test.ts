...
describe('useCategorizationAgent', () => {
	...
	it('sends the applied categories in the candidate shape and drops them from the suggestions', async () => {
		...
		expect(mockPostAgentInstance).toHaveBeenCalledWith(
			expect.objectContaining({
				context: expect.objectContaining({
					...
					candidateCategories: JSON.stringify(candidateCategories),
				}),
			})
		);
	...
	it('sends the applied tags and drops them from the suggestions regardless of case', async () => {
		const {fakeEventSource, result} = await renderAgent(
			ECategorizationAgent.GENERATE_TAGS
		);
